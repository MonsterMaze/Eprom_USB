...
void QtGui::WriteEpromSlot(void)
{
    ...
    qDebug() << "=== Valores desde la interfaz ===";
    qDebug() << "entryProgVcc texto:" << entryProgVcc->text();
    qDebug() << "entryVpp texto:" << entryVpp->text();
    qDebug() << "VCC convertido:" << entryProgVcc->text().toDouble();
    qDebug() << "VPP convertido:" << entryVpp->text().toDouble();

// Convertir correctamente coma/punto
    QString vccText = entryProgVcc->text().replace(',', '.');
    QString vppText = entryVpp->text().replace(',', '.');

    double vcc = vccText.toDouble();
    double vpp = vppText.toDouble();


    qDebug() << "VCC:" << vcc << " | VPP:" << vpp;
